...
void setup(){
	...
	ledcAttachPin(motorA1,0);ledcSetup(0,10000,12);
    ledcAttachPin(motorA2,1);ledcSetup(1,10000,12);
	ledcAttachPin(motorB1,2);ledcSetup(2,10000,12);
    ledcAttachPin(motorB2,3);ledcSetup(3,10000,12);

    ledcAttachPin(fan,2);ledcSetup(2,10000,12);
...
void loop(){
	...
	if(abs(pid)<vlin){
		...
		if(pid<0){
			pid=map(pid,-2*vlin,-vlin,0,vlin);
			pwm(0,0);pwm(1,vlin);
			pwm(3,0);pwm(2,vlin-pid);
		}else{
			pid=map(pid,2*vlin,vlin,0,vlin);
			pid-=vlin;
			pwm(1,0);pwm(0,vlin-pid);
			pwm(2,0);pwm(3,vlin);
		}
