...
void lecturasensores(){
	...
	if(sensorcentral<=-3){ sensor[0]=adc1_get_raw(ADC1_CHANNEL_4);   //32
	}else{	sensor[0]=0;	}
	
	if(sensorcentral<=-2){ sensor[1]=adc1_get_raw(ADC1_CHANNEL_7);   //35
	}else{  sensor[1]=0;	}
	
	if(sensorcentral<=-1){ sensor[2]=adc1_get_raw(ADC1_CHANNEL_6);   //34
	}else{  sensor[2]=0;	}

	if((sensorcentral<=0)&&(sensorcentral>=-4)){  sensor[3]=adc1_get_raw(ADC1_CHANNEL_3);   //39
	}else{  sensor[3]=0;	}
	
	if((sensorcentral<=1)&&(sensorcentral>=-3)){  sensor[4]=adc1_get_raw(ADC1_CHANNEL_0);   //36
	}else{  sensor[4]=0;	}

	if((sensorcentral<=2)&&(sensorcentral>=-2)){  sensor[5]=adc1_get_raw(ADC1_CHANNEL_5);   //33
	}else{  sensor[5]=0;	}


	if((sensorcentral<=3)&&(sensorcentral>=-1)){  adc2_get_raw( ADC2_CHANNEL_4,ADC_WIDTH_12Bit,&sensor[6]); sensor[6]=4095-sensor[6]; //13
	}else{  sensor[6]=0;	}

	if((sensorcentral<=5)&&(sensorcentral>=0)){   adc2_get_raw(ADC2_CHANNEL_3,ADC_WIDTH_12Bit,&sensor[7]); sensor[7]=4095-sensor[7]; //15
	}else{  sensor[7]=0;	}
		
	if(sensorcentral>=1){	adc2_get_raw(ADC2_CHANNEL_2,ADC_WIDTH_12Bit,&sensor[8]); sensor[8]=4095-sensor[8]; //2
	}else{  sensor[8]=0; }

	if(sensorcentral>=2){	adc2_get_raw(ADC2_CHANNEL_0,ADC_WIDTH_12Bit,&sensor[9]); sensor[9]=4095-sensor[9]; //4
	}else{  sensor[9]=0; }

	if(sensorcentral>=3){	adc2_get_raw(ADC2_CHANNEL_5,ADC_WIDTH_12Bit,&sensor[10]); sensor[10]=4095-sensor[10]; //12
	}else{  sensor[10]=0; }
  ...
  //  Serial.println(dato);
    ...
    if(num!=0){ 
       // posicion=(posicion*0.8)+(posicion2*0.2);        
		 posicion=posicion2;
		 sensorcentral=(int)(posicion/100);
...
void loop(){
	...
	if(abs(posicion)>=500){
		if(posicion<0){
			pwm(0,0);pwm(1,vlin);
			pwm(3,0);pwm(2,vlin/4);
		}else{
			pwm(1,0);pwm(0,vlin/4);
			pwm(2,0);pwm(3,vlin);
		}
	}else{
